...
import {
  ...
  Mail, Phone, RefreshCw, AlertCircle, Sparkles, MapPin,
  CheckSquare, Square, Layers, Heart, Calendar, Briefcase,
  HeartHandshake, Wallet, Home, Award, Send, Vote, LayoutTemplate
} from 'lucide-react';
...
const HEAD_PERMISSION_CATEGORIES = [
  {
    category: 'Members & Matrimonial',
    icon: Users,
    color: 'from-blue-500/20 to-indigo-500/20 text-indigo-400',
    permissions: [
      { key: 'canViewDashboard', label: 'Dashboard Access', desc: 'Can access dashboard and view analytics' },
      { key: 'canViewMembers', label: 'View Members', desc: 'Can view community member roster' },
      { key: 'canAddMembers', label: 'Add Members', desc: 'Can manually register new community members' },
      { key: 'canEditMembers', label: 'Edit Members', desc: 'Can modify member details and approval status' },
      { key: 'canRemoveMembers', label: 'Remove Members', desc: 'Can revoke or delete members' },
      { key: 'canExportMembers', label: 'Export Members', desc: 'Can download CSV/Excel member roster' },
      { key: 'canViewProfiles', label: 'Matrimonial Profiles', desc: 'Can view and search matrimonial profiles' },
      { key: 'canApproveProfiles', label: 'Approve Matrimonial Profiles', desc: 'Can verify & approve bride/groom profiles' },
      { key: 'canViewCensus', label: 'Census Roster', desc: 'Can review family census data' }
    ]
  },
  {
    ...
    permissions: [
      { key: 'canViewEvents', label: 'View Events', desc: 'Can view community events and meetings' },
      { key: 'canCreateEvents', label: 'Create Events', desc: 'Can publish new events and ceremonies' },
      { key: 'canEditEvents', label: 'Edit Events', desc: 'Can update event schedules and venues' },
      { key: 'canDeleteEvents', label: 'Delete Events', desc: 'Can cancel or remove events' },
      { key: 'canManageBookings', label: 'Event Bookings & RSVPs', desc: 'Can manage attendee registrations' },
      { key: 'canViewInvitations', label: 'View Invitations', desc: 'Can view digital ceremonial cards' },
      { key: 'canCreateInvitations', label: 'Create Invitations', desc: 'Can create community invitations' },
      { key: 'canViewElections', label: 'Election Commission', desc: 'Can oversee elections and ballots' }
    ]
  },
  {
    category: 'Finance & Donations',
    icon: Wallet,
    color: 'from-emerald-500/20 to-teal-500/20 text-emerald-400',
    permissions: [
      { key: 'canViewFunds', label: 'View Samaj Funds', desc: 'Can view funds balance and ledgers' },
  ...
  {
    category: 'Facilities & Communications',
    icon: Home,
    color: 'from-amber-500/20 to-orange-500/20 text-amber-400',
    permissions: [
      { key: 'canViewDharmashala', label: 'Dharmashala Bookings', desc: 'Can view guest house bookings' },
      { key: 'canManageDharmashala', label: 'Manage Dharmashalas', desc: 'Can approve and allocate rooms' },
      { key: 'canViewSocial', label: 'Community Feed & Groups', desc: 'Can view and moderate social discussions' },
      { key: 'canViewDirectory', label: 'Professional Directory', desc: 'Can review verified business listings' },
      { key: 'canViewObituary', label: 'Obituary Desk', desc: 'Can view and post condolence messages' },
      { key: 'canSendNotifications', label: 'Send Announcements & Push', desc: 'Can send broadcast notifications' },
      { key: 'canViewHomeContent', label: 'Community Home Content', desc: 'Can manage banner notices and spotlights' },
      { key: 'canViewReports', label: 'Community Reports', desc: 'Can generate and review activity reports' }
    ]
  }
];

export const HeadSubHeadManagement = () => {
  ...
  const [form, setForm] = useState({
    ...
    phone: '',
    password: '',
    designation: isLocalHead ? 'Local Sub-Head' : 'Community Sub-Head',
  ...
  const fetchSubHeads = async () => {
    ...
    try {
      ...
      const res = await axiosPrivate.get(`/head/sub-heads?${params.toString()}`);
  ...
  // Check whether parent has right to grant a specific permission
  const isPermissionGrantable = (permKey) => {
    if (isMainHead) return true;
    return parentPermissions[permKey] === true;
  };

  const openCreateModal = () => {
    setEditingSubHead(null);
    const defaultPermissions = {};
    HEAD_PERMISSION_CATEGORIES.forEach(cat => {
      cat.permissions.forEach(p => {
        defaultPermissions[p.key] = p.key === 'canViewDashboard' || p.key === 'canViewMembers';
      });
    });
    ...
    setForm({
      ...
      designation: isLocalHead ? 'Local Sub-Head' : 'Community Sub-Head',
      department: 'Operations',
      headPermissions: defaultPermissions
    });
    setIsModalOpen(true);
  };
  ...
  const openEditModal = (subHead) => {
    ...
    const permissions = { ...(subHead.headPermissions || {}) };
    HEAD_PERMISSION_CATEGORIES.forEach(cat => {
      cat.permissions.forEach(p => {
    ...
    setForm({
      ...
      phone: subHead.phone || '',
      password: '',
      designation: subHead.designation || (isLocalHead ? 'Local Sub-Head' : 'Community Sub-Head'),
  ...
  const handleTogglePermission = (permKey) => {
    if (!isPermissionGrantable(permKey)) {
      showToast('You can only assign permissions that you yourself have.', 'warning');
      return;
    }
    setForm(prev => ({
      ...prev,
  ...
  const handleToggleCategory = (category, selectAll) => {
    setForm(prev => {
      const updated = { ...prev.headPermissions };
      category.permissions.forEach(p => {
        if (isPermissionGrantable(p.key)) {
          updated[p.key] = selectAll;
        }
      });
      return { ...prev, headPermissions: updated };
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingSubHead) {
        ...
        await axiosPrivate.put(`/head/sub-heads/${editingSubHead._id}`, payload);
        showToast('Sub-Head updated successfully!', 'success');
      } else {
        await axiosPrivate.post('/head/sub-heads', form);
        showToast(`${isLocalHead ? 'Local' : 'Community'} Sub-Head created successfully!`, 'success');
      }
      setIsModalOpen(false);
      fetchSubHeads();
    } catch (err) {
      showToast(err.response?.data?.message || 'Failed to save Sub-Head', 'error');
    } finally {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      await axiosPrivate.patch(`/head/sub-heads/${id}/status`);
  ...
  const handleDelete = async (id, name) => {
    if (!window.confirm(`Are you sure you want to deactivate Sub-Head ${name}?`)) {
      return;
    }
    try {
      await axiosPrivate.delete(`/head/sub-heads/${id}`);
      showToast('Sub-Head deactivated successfully', 'success');
      fetchSubHeads();
    } catch (err) {
    ...
    <div className="space-y-8 pb-24 font-sans max-w-7xl mx-auto">
      ...
      <div className="relative bg-white border-b border-gray-100 -mt-6 -mx-6 px-8 py-10 shadow-sm overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div>
              <div className="flex items-center gap-2.5">
                <h1 className="text-3xl font-black text-gray-900 tracking-tight">
                  {isLocalHead ? 'Local Sub-Head Management' : 'Community Sub-Head Management'}
                </h1>
                <span className="px-3 py-1 rounded-full text-xs font-black bg-purple-100 text-purple-700 tracking-wide uppercase">
                  {isLocalHead ? `${headUser?.city || 'Local'} Unit` : (headUser?.community || 'Community')}
                </span>
              </div>
              <p className="text-gray-500 font-medium text-sm mt-1">
                {isLocalHead
                  ? `Create and manage Sub-Heads for ${headUser?.city || 'your location'} with tailored module powers.`
                  : 'Create subordinate leaders under your Samaj Community and assign granular permissions.'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <Plus size={18} /> Create {isLocalHead ? 'Local' : 'Community'} Sub-Head
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-purple-600 font-semibold mt-1">Under your jurisdiction</p>
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-emerald-600 font-semibold mt-1">Authorized login credentials</p>
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-amber-600 font-semibold mt-1">Suspended permissions</p>
          </div>
      ...
      {loading ? (
        <div className="bg-white rounded-2xl p-16 border border-gray-100 text-center">
          ...
          <p className="text-gray-500 font-semibold text-sm">Loading Sub-Heads...</p>
        </div>
      ) : subHeads.length === 0 ? (
        <div className="bg-white rounded-2xl p-16 border border-gray-100 text-center space-y-4">
          ...
          <h3 className="text-lg font-bold text-gray-900">No Sub-Heads Found</h3>
          <p className="text-gray-500 text-sm max-w-md mx-auto">
            {searchQuery
              ? 'No accounts match your search filter.'
              : `Create your first ${isLocalHead ? 'Local' : 'Community'} Sub-Head to delegate management of events, funds, or members.`}
          </p>
          <button
            onClick={openCreateModal}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {subHeads.map((subHead) => {
            const perms = subHead.headPermissions || {};
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto no-scrollbar">
                      {allowedCount === 0 ? (
                        ...
                        HEAD_PERMISSION_CATEGORIES.flatMap(cat => cat.permissions)
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-black">
                      {editingSubHead
                        ? `Edit ${isLocalHead ? 'Local' : 'Community'} Sub-Head`
                        : `Create New ${isLocalHead ? 'Local' : 'Community'} Sub-Head`}
                    </h3>
                    <p className="text-xs text-purple-200/70 mt-0.5">
                      {isLocalHead
                        ? `Scoped to ${headUser?.city || 'your city'}. Assign specific module permissions.`
                        : 'Assign specific module powers and login credentials.'}
                    </p>
                  </div>
                </div>
              ...
              <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        placeholder="e.g. Assistant Samaj Secretary"
                    ...
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1.5">Department / Wing</label>
                      <input
                        type="text"
                        placeholder="e.g. Youth & Cultural Wing"
                ...
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <h4 className="text-xs font-black tracking-widest text-gray-400 uppercase flex items-center gap-2">
                      ...
                      Granular Module Permissions & Powers
                    </h4>
                  </div>

                  <div className="space-y-6">
                    {HEAD_PERMISSION_CATEGORIES.map((cat) => {
                      const CatIcon = cat.icon;
                      const grantablePermissions = cat.permissions.filter(p => isPermissionGrantable(p.key));
                      if (grantablePermissions.length === 0) return null;

                      const allSelected = grantablePermissions.every(p => form.headPermissions[p.key]);
                        ...
                        <div key={cat.category} className="bg-gray-50/70 rounded-2xl p-5 border border-gray-200/70 space-y-3">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2">
                            {grantablePermissions.map((p) => {
                              const isChecked = form.headPermissions[p.key] === true;
                ...
                <div className="pt-6 border-t border-gray-100 flex items-center justify-end gap-3 shrink-0">
                  ...
                  >
                    {isSubmitting ? 'Saving Sub-Head...' : editingSubHead ? 'Update Sub-Head' : 'Create Sub-Head'}
                  </button>
                </div>
              </form>
            </motion.div>
...
export default HeadSubHeadManagement;
