...
export default function InvitationHomePage() {
  ...
  const [openedCardIds, setOpenedCardIds] = useState(() => {
    try {
      const saved = localStorage.getItem('merisamaj_opened_invitation_cards');
  ...
  const handleToggleOpenCard = (id, isOpen) => {
    setOpenedCardIds(prev => {
      ...
      try {
        localStorage.setItem('merisamaj_opened_invitation_cards', JSON.stringify([...next]));
      } catch (e) {}
      return next;
    });
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50/70 flex flex-col font-sans select-none pb-44 sm:pb-36">
      ...
      <div className="bg-white/80 backdrop-blur-md border-b border-purple-100/30 px-3.5 sm:px-4 py-2.5 sm:py-3 sticky top-[56px] z-20 shadow-xs space-y-2">
        ...
        <div className="flex items-center gap-1 p-1 bg-slate-100/80 rounded-2xl">
          {[
            ...
            <button
              ...
              className={`flex-1 py-2 sm:py-2.5 rounded-xl text-[12px] sm:text-[12.5px] font-black transition-all press-scale flex items-center justify-center gap-1.5 ${
        ...
        {/* Date Filter Pills */}
        <div className="flex items-center gap-1.5 -mx-3.5 sm:-mx-4 px-3.5 sm:px-4 overflow-x-auto scrollbar-hide pb-0.5">
          <span className="text-[10px] sm:text-[10.5px] font-extrabold uppercase tracking-wider text-slate-400 shrink-0 flex items-center gap-1 pl-0.5">
          ...
          {dateFilters.map(filter => (
            <button
              ...
              className={`whitespace-nowrap px-3 sm:px-3.5 py-1.5 rounded-xl text-[10.5px] sm:text-[11px] font-bold transition-all press-scale border shrink-0 ${
      ...
      <div className="flex-1 overflow-y-auto px-3.5 sm:px-4 py-3 sm:py-4 space-y-4 max-w-4xl mx-auto w-full">
        <AnimatePresence>
          {filteredInvitations.map(inv => {
            ...
            const isCardOpen = openedCardIds.has(cardId);
            return (
              <EnvelopeInvitationCard
                key={cardId}
                inv={inv}
                isOpen={isCardOpen}
                onToggleOpen={handleToggleOpenCard}
      ...
      <button 
        ...
        className="fixed bottom-[96px] sm:bottom-[92px] right-4 sm:right-5 w-12 h-12 sm:w-13 sm:h-13 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-purple-500/25 active:scale-90 transition-transform z-30 press-scale hover:scale-105"
      >
